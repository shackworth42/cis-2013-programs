...
floatFinPts = parseFloat(prompt("Please enter your Final Exam grade (0-35):"))

floatTotalPts = parseFloat(floatFinPts + floatHwPts + floatMidPts);

intGradeOption = parseInt(prompt('Please type "1" if you the class is Pass/Fail or \n "2" if you wish to recieve a letter grade'))

if (intGradeOption===1)
{
    if(floatTotalPts >= 80){
        stringFinalGrade = "A somewhat cowardly pass"
    }else { stringFinalGrade = "A Fail, even though you audited"}
}else{
    if (floatTotalPts > 100) {
        stringFinalGrade = "Worthy of a full ride to Harvard"
